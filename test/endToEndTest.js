...
it("should add a handler, connect to EventStore, emit an event and close the connection", async () => {
    ...
    let promise = new Promise(function(resolve, reject){
        ...
        subscriber.createConnection({}).then(() =>
            ...
            esConnection.once('closed', () => logger.info("publishing connection closed"));
    ...
    it("should consume events from category stream", async () => {
        let subscriber2 = require("../app/eventstore-stream-subscriber.js")
        
        subscriber2.registerHandler("OrderCreated", (subscription, evt) => {
            logger.info("OrderCreated event arrived!");
            logger.info(JSON.stringify(evt));
        });
        subscriber2.registerHandler("ItemAdded", (subscription, evt) => {
            logger.info("OrderCreated event arrived!");
            logger.info(JSON.stringify(evt));
        });
        subscriber2.configure({ resolveLinkTos : true});
        
        const streamName = "$ce-Order";
        logger.info("Subscribing to " + streamName);
        subscriber2.createConnection({}).then(() => {
            logger.info("Connection for " + streamName +" created!")
            subscriber2.catchupAndSubscribeToStream(streamName).then(() => logger.info("Subscribed to " + streamName))
    ...
    it("should readstream forward", async () => {
        let subscriber3 = require("../app/eventstore-stream-subscriber.js");
        subscriber3.registerHandler("OrderCreated", (subscription, evt) => {
            logger.info("OrderCreated event arrived!");
            logger.info(JSON.stringify(evt));
        });
        ...
        subscriber3.configure({ resolveLinkTos : true});

        const streamName = "$ce-Order";
        await subscriber3.readWholeStreamFromStart(streamName, {}, "c3")
