...
module.exports = () => {
  ...
  const eventstoreHelper = {
    ...
    getEndpoint: () => {
      if (process.env.EVENTSTORE_STREAM_SUB_HOST)
        return `tcp://${process.env.EVENTSTORE_STREAM_SUB_HOST}:${
          process.env.EVENTSTORE_STREAM_SUB_PORT
        }`;
      return `tcp://${configuration.eventstoreHost}:${
        configuration.eventstorePort
      }`;
    },
    connection: null
  };
  ...
  function configure(config) {
    if (typeof config !== "object") {
      logger.handleLog(
        ...
        "argument 'config' must be an object"
      );
  ...
  const eventAppeared = (subscription, event) => {
    ...
    if (event.eventType) {
      ...
      logger.handleLog(
        ...
        "Event appeared, but no eventType could be detected."
      );
    }
  };
  const subscriptionDropped = (subscription, reason, error) =>
    logger.handleLog(
      LogLevels.ERROR,
      "Subscription dropped " + reason + " error: " + error
    );
  ...
  function createConnection(settings, connectionName) {
    ...
    if (!settings)
      logger.handleLog(
        logger.logLevels.FATAL,
        "parameterSettings must be an object"
      );
    let endpoint = eventstoreHelper.getEndpoint();
    logger.handleLog(
      logger.logLevels.INFO,
      "Creating connection to host " + endpoint
    );
    let conn = client.createConnection(
      settings,
      eventstoreHelper.getEndpoint(),
      connectionName
    );
    ...
    return new Promise(function(resolve, reject) {
      if (error) {
        ...
        conn.once("connected", tcpEndpoint => {
          logger.handleLog(
            logger.logLevels.INFO,
            `connected to Eventstore - host: ${tcpEndpoint.host} port: ${
              tcpEndpoint.port
            }`
          );
          resolve();
  ...
  function closeConnection() {
    ...
    logger.handleLog(
      logger.logLevels.INFO,
      "Closing connection to host " + endpoint
    );
  ...
  async function readStreamForward(stream, settings, conn) {
    ...
    while (!result.isEOS) {
      await readEventSliceForward(conn, stream, result.next, eh).then(
        async r => (result = r)
      );
    }
  }
  ...
  async function readEventSliceForward(conn, stream, startPos, eventHandler) {
    let rs = await conn.readStreamEventsForward(
      stream,
      startPos,
      2,
      true,
  ...
  function catchupAndSubscribeToStream(stream) {
    ...
    logger.handleLog(
      logger.logLevels.INFO,
      "cred: " + JSON.stringify(eventstoreHelper.getCredentials())
    );
    return new Promise(function(resolve, reject) {
      if (!conn) {
        reject("Connection not initialized, call createConnection() first");
